(ns welfare.actor
  "WelfareActor — the ISCO-08 1344 social welfare manager actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one welfare management operation request
  ...
  The unconditional invariant: the WelfareAdvisor can never directly commit
  a record or dispatch a robot action the WelfareGovernor refuses — every
  commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [welfare.advisor :as advisor]
            [welfare.governor :as governor]
            [welfare.store :as store]))

(defn build-graph
  "Build a compiled WelfareActor graph. `store` implements
  `welfare.store/Store`. `advisor` implements `welfare.advisor/Advisor`
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:agency-id (:agency-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
